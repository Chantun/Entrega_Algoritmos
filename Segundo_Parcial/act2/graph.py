...
class Graph(List):
    ...
    class __nodeVertex:
        ...
        def __str__(self):
            return f"{self.value}, {self.other_values}"
    ...
    def insert_vertex(self, value: Any, other_value: Optional[Any] = None) -> None:
        node_vertex = Graph.__nodeVertex(value, other_value)
        self.append(node_vertex)

    def insert_edge(
        self, origin_vertex: Any, destination_vertex: Any, weight: int
    ) -> None:
                    ...
                    if graph[vertex_pos].value == destination:
                        ...
                        for edge in graph[vertex_pos].edges:
                            ...
                            if not graph[destination_edge_pos].visited:
                                result = __exist_path(
                                    graph,
                                    graph[destination_edge_pos].value,
                                    destination,
                                )
                                if result:
                                    break
        ...
        while no_visited.size() > 0:
            ...
            for edge in edges:
                ...
                if pos is not None:
                    if pos is not None:
                        if (
                            costo_nodo_actual + edge.weight
                            < no_visited.elements[pos][0]
                        ):
                            no_visited.elements[pos][1][2] = value[1][0]
                            no_visited.change_priority(
                ...
                if origin != destination:
                    ...
                    if "-" not in vertex_origin and "-" not in vertex_destination:
                        ...
                        forest.append(
                            vertex_origin
                            + ";"
                            + f"{edge[1][0]}-{vertex_destination}-{edge[0]}"
                        )
