...
class BinaryTree:
    ...
    def in_order(self):
        def __in_order(root):
            if root is not None:
                ...
                print(f"{root.value}, {root.other_values['defeated_by']}")
    ...
    def in_order_by_param(self, param, value):
        def __in_order_by_param(root):
            if root is not None:
                __in_order_by_param(root.left)
                if root.other_values[param] == value:
                    print(f"{root.value}")
                __in_order_by_param(root.right)

        if self.root is not None:
            __in_order_by_param(self.root)

    def count_hunters(self, n):
        def __count_hunters(root):
            auxList = []
            if root is not None:
                defeated_by = root.other_values["defeated_by"]
                if defeated_by is not None:
                    auxList.append(defeated_by)
                auxList.extend(__count_hunters(root.left))
                auxList.extend(__count_hunters(root.right))
            return auxList

        if self.root is not None:
            finalList = __count_hunters(self.root)
        return Counter(finalList).most_common(n)

    def modify_param(self, node_value, param, new_value):
        node = self.delete(node_value)
        if node != (None, None):
            node[1][param] = new_value
